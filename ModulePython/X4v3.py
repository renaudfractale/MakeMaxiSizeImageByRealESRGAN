def X4Size(im,imF,step,factor,V,model,Image,ImageChops):
    ...
    if step!=width :
        imM2 = Image.new(mode="RGBA", size=(height*factor, width*factor),color=(0, 0, 0, 255))
        #B2
        for i_height in range(int(-step/2), height-1, int(step/2)):
        ...
        for i_height in range(int(-step/2), height-1, step):
            for i_width in range(0, width-1, step):
                print("B3 : " + str(i_height)+"x"+str(i_width)) 
                left = i_width
                top = i_height
                right = i_width+step
                bottom = i_height+step
