def X4Size(im,imF,step,factor,V,model,Image,ImageChops):
    ...
    if step!=width :
        ...
        #B2
        for i_height in range(0, height, int(step/2)):
            for i_width in range(0, width, int(step/2)):
                if ((int(i_width/int(step/2)) % 2 ) == 0 or (int(i_height/int(step/2)) % 2) == 0) and (i_height * i_width !=0):
